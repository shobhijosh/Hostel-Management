...
const EditStudent = ({ fetchRooms }) => {
    ...
    // Store the room Type of the roomNum
    ...
    // Fetch the student detail by object id
    const findStudent = async () => {
        const json = sessionStorage.getItem('students')
        const allStudents = [JSON.parse(json)]
        const student = allStudents[0].find(student=> student._id === id)
        setRoomNum(student.roomNo)
        setStudent(student)
        // setLoading(true)
        // const response = await fetch(`http://localhost:5000/api/findStudent/${id}`, {
        //     method: 'GET',
        // })
        // const json = await response.json()
        // if (json.success) {
        //     setRoomNum(json.data.roomNo)
    ...
    useEffect(() => {
        // findStudent()
        Promise.all([findStudent()]).then(()=> setLoading(false)).catch((error)=> {
            console.log(error);
            setLoading(false);
        })
    }, [])
        ...
        if (newRoomNum !== '') {
            ...
            if (selected) {
                ...
                setStudent(prevStudent => (
                    { ...prevStudent, roomNo: newRoomNum, roomType: selected.roomType }
                ))
            }
        }
        else {
            setStudent((prevStudent) => ({ ...prevStudent, roomNo: '', roomType: '' }))
        }
    }

    // Edit the student details into the database also
    const editStudentDetails = async () => {
        const body = JSON.stringify(student)
        const response = await fetch(`http://localhost:5000/api/students/editStudent/${id}`, {
            method: 'PUT',
            headers: {
        ...
        const json = await response.json()
        if (json.success) sessionStorage.setItem('students', JSON.stringify(json.data))
        return json
    }
    ...
    const allocateRoom = async () => {
        const body = (roomNum === '') ? {status:'Available'} : {status:'Alloted'}
    ...
    // Add fee details to the fee records if the room is alloted otherwise delete the prev alloted room if the student is unassigned the room
    const addFeeDetails = async () => {
        ...
        if(roomNum !==''){
            ...
            const feeID = fees.find((fee)=> students.map((student)=> student._id === fee.studentDetails))._id
            console.log(feeID);
            const response = await fetch(`http://localhost:5000/api/fees/deleteFee/${feeID}`,{method:'DELETE'})
            const json = await response.json()
    ...
    const handleSubmit = async (e) => {
        ...
        const json = await editStudentDetails()
        await addFeeDetails()
        setLoading(false)
                    ...
                    <section className="add-student-form-container">
                        <form className="add-student-form">
                            ...
                            <div className="form-group full-width">
                                ...
                                <input type='text' name='roomType' value={student.roomType || ''} onChange={onRoomNumChange} disabled />
                                {/* {roomDetails.map((prop)=>{
                            ...
                            <div className="form-actions full-width">
                                ...
                                <button type="submit" className="submit-btn" onClick={handleSubmit}>Edit Student</button>
                            </div>
                        </form>
                    </section>
                </main>
